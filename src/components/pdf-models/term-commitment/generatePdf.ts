...
export function generatePDF(dadosTce: CreatedTermCommitment): Promise<Blob> {
  return new Promise((resolve, reject) => {
    ...
    // Função para adicionar o logo/texto ao lado + background brasão
    ...
    // Aguarde o carregamento da imagem de fundo
    backgroundImg.onload = () => {
      ...
      // Resto da geração do PDF usando dadosTce...
      doc.setFontSize(10);
      doc.setFont('arial', 'bold');
      ...
      // Exibir o PDF em um blob URL
      const pdfOutput = doc.output('blob');
      resolve(pdfOutput); // Resolve a Promise com o blobUrl
    };

    // Trata erro de carregamento da imagem
    backgroundImg.onerror = (error) => {
      reject(new Error('Erro ao carregar a imagem de fundo: ' + error));
    };
  });
}
