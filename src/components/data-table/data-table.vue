...
const mapInternshipProcesses = () => {
  mappedInternshipProcesses.value = internshipProcessRegisters.value.map(
    (internshipProcess) => ({
      aluno: internshipProcess.user.name,
      matricula: internshipProcess.user.registration,
      concedente: internshipProcess.termCommitment.razaoSocialConcedente,
...
function defineColorStatus(status: string): string | undefined {
  switch (status.toUpperCase()) {
    case InternshipProcessStatus.CONCLUIDO:
      return 'green';
    case InternshipProcessStatus.EM_ANDAMENTO:
      return 'orange';
    case InternshipProcessStatus.RECUSADO:
      return 'red';
  }
}
...
function formatarDataBrasileira(data: string | Date): string {
  ...
  const dataUTC = new Date(
    fortmatDate.getUTCFullYear(),
    fortmatDate.getUTCMonth(),
    fortmatDate.getUTCDate(),
  );
